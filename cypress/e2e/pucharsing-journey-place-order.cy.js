...
describe('Tela de fechamento do pedido', () => {
    ...
    it('Válida adicionar instruções de entrega', () => {
       // Clique em "Adicionar instruções de entrega"
        ...
        cy.get('.a-size-base.a-color-base.ma-saved-property-type-text.a-text-bold')
    ...
    it('Válida mensagem de erro ao tentar adicionar cupom incorreto durante a seleção do metodo de pagamento', () => {
        ...
        cy.get('.pmts-error-message-inline > .a-box > .a-box-inner > .a-alert-content > p')
        .should('have.text', 'É necessário um cartão de presente ou código promocional.')
    });
    it('Deve fechar o pedido com o Pix', () => {
        ...
        cy.intercept('POST', '/gp/buy/shared/handlers/async-continue.html/ref=chk_pay_change_coll*').as('asyncContinue');
        cy.get('#spc-orders')
        .should('be.visible');
        cy.wait('@asyncContinue');
        
        // Válida botão de finalizar compra
        cy.get('input[name="placeYourOrder1"]')
        .should('be.visible')
    });
